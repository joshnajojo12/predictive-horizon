...
class PIDController:
    """Calculates actuator angular velocity commands based on visual residuals."""
    def __init__(self, kp: float = 1.2, ki: float = 0.01, kd: float = 0.1):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def calculate_correction(self, residual_x: float, residual_y: float, 
                             fov_x: float, fov_y: float, res_x: int, res_y: int, dt: float) -> tuple[float, float]:
        """
        Converts pixel residual to angular error, applies PID, returns required angular velocity.
        """
        if dt <= 0:
            return 0.0, 0.0
        ...
        # Integral
        self.integral_az += err_az * dt
        self.integral_el += err_el * dt
        ...
        return p_az + i_az + d_az, p_el + i_el + d_el
